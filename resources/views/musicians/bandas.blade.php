    ...
    <div class="bg-darker px-12 py-10 border-b border-peach/10">
        <div class="ml-[16%]">
            <h1 class="font-serif font-black text-cream leading-tight text-[clamp(2rem,4vw,3.2rem)]">
                Bandas<br><span class="text-peach">disponibles</span>
            </h1>
            <p class="text-muted mt-2 text-sm">Encuentra bandas de toda España</p>
        </div>
    </div>

    <!-- Filtros -->
    <div class="px-12 -translate-y-6">
        <form method="GET" action="{{ route('bandas') }}"
    ...
    <div class="px-12 pb-12 max-w-[1280px] mx-auto w-full">
        ...
        @if($bands->isEmpty())
            <div class="text-center py-20 text-muted">
                ...
                <p class="text-lg mt-4">No se encontraron bandas con esos filtros.</p>
            </div>
        @else
            <p class="text-xs text-muted font-semibold uppercase tracking-widest mb-6">
                {{ $bands->total() }} bandas encontradas
            </p>
                ...
                @foreach($bands as $band)
                    <a href="{{ route('profile.show', $band->username) }}"
                        ...
                        <div class="p-5 flex flex-col gap-3 flex-1">
                            <div class="flex items-center gap-3">
                                @if($band->photo)
                                    ...
                                    alt="{{ $band->name }}">
                                @else
                                    <div class="rounded-[14px] bg-gradient-to-br from-peach to-brand flex items-center justify-center font-serif text-xl font-bold text-white shrink-0"
                                        style="width:52px;height:52px">
                                        {{ strtoupper(substr($band->name, 0, 1)) }}
                                    </div>
                                @endif
                                <div>
                                    <p class="font-bold text-[.95rem] text-dark leading-tight">{{ $band->full_name ?? $band->name }}</p>
                                    <p class="text-xs text-muted mt-0.5 font-medium">{{ '@' . $band->username }}</p>
                                </div>
                            </div>
                        ...
                        <div class="px-5 py-3 border-t border-peach/20 flex items-center justify-between">
                            <span class="text-xs font-bold px-2.5 py-1 rounded-md inline-flex items-center gap-1.5 tracking-wide bg-peach/15 text-[#9a3a00] border border-peach/35">
