...
function toggleDropdown(id) {
    ...
    const allDropdowns = ['following-dropdown', 'followers-dropdown'];
...
document.addEventListener('click', function(e) {
    if (!e.target.closest('[onclick]')) {
        document.getElementById('following-dropdown').style.display = 'none';
        document.getElementById('followers-dropdown').style.display = 'none';
    }
});
</script>
@endpush
...
<x-app-layout>
    <main class="max-w-4xl mx-auto px-6 py-10 w-full">
        @if(session('status') === 'profile-updated')
            <div
                style="background:rgba(34,197,94,.1); border:1px solid rgba(34,197,94,.3); color:#16a34a; padding:.75rem 1rem; border-radius:12px; margin-bottom:1.5rem;">
                Perfil actualizado correctamente.
            </div>
        @endif

        {{-- Banner de banda --}}
        @if($user->account_type === 'band' && $user->banner)
            <div
                style="width:100%; height:180px; border-radius:20px; overflow:hidden; margin-bottom:1.5rem; border:1px solid rgba(255,193,147,.15);">
                <img src="{{ Storage::url($user->banner) }}" style="width:100%; height:100%; object-fit:cover;">
            </div>
        @elseif($user->account_type === 'band')
            <div
                        style="width:100%; height:180px; border-radius:20px; margin-bottom:1.5rem; background:linear-gradient(135deg,var(--mid),var(--dark)); border:1px solid rgba(255,193,147,.15); display:flex; align-items:center; justify-content:center;">
                        <p style="color:rgba(255,237,206,.2); font-size:.85rem;">Sin banner</p>
                    </div>
        @endif
            
            <!-- Cabecera -->
            <div
                style="background:var(--mid); border-radius:20px; padding:2rem; margin-bottom:1.5rem; border:1px solid rgba(255,193,147,.15);">
                            ...
                            @endif
                                    ...
                                    @else
                                        ...
                                        @if($followStatus === 'accepted')
                                            <div style="display:flex; gap:.5rem;">
                                                <form method="POST" action="{{ route('follow.unfollow', $user) }}">
                                                    @csrf
                                                    <button
                                                        style="padding:.5rem 1.25rem; border-radius:10px; border:1.5px solid var(--muted); color:var(--muted); background:transparent; font-size:.85rem; font-weight:600; cursor:pointer;"
                                        ...
                                        @elseif($followStatus === 'pending')
                                            <form method="POST" action="{{ route('follow.unfollow', $user) }}">
                                                @csrf
                                                <button
                                                    style="padding:.5rem 1.25rem; border-radius:10px; border:1.5px solid rgba(255,193,147,.3); color:rgba(255,237,206,.4); background:transparent; font-size:.85rem; font-weight:600; cursor:pointer;">
                                                    Solicitud enviada
                                                </button>
                                            </form>
                                        @else
                                            <form method="POST" action="{{ route('follow.send', $user) }}">
                                                @csrf
                                                <button
                                                    style="padding:.5rem 1.25rem; background:var(--red); color:#fff; border-radius:10px; font-size:.85rem; font-weight:600; border:none; cursor:pointer; transition:background .2s;"
                                                    onmouseover="this.style.background='var(--salmon)'" onmouseout="this.style.background='var(--red)'">
                                                    + Seguir
                                                </button>
                                            </form>
                                ...
                                <div style="display:flex; flex-wrap:wrap; gap:1rem; margin-top:1rem; font-size:.85rem; color:var(--muted);">
                                    ...
                                    @if($user->general_level)
                                        <span
                                            style="padding:.2rem .75rem; border-radius:999px; background:rgba(255,55,55,.1); color:var(--red); border:1px solid rgba(255,55,55,.2);">
                                            {{ ['', 'Principiante', 'Básico', 'Intermedio', 'Avanzado', 'Profesional'][$user->general_level] }}
                                        </span>
                                    @endif
                                    @if($user->account_type === 'musician')
                                        <span
                                            style="padding:.2rem .75rem; border-radius:999px; background:rgba(255,193,147,.1); border:1px solid rgba(255,193,147,.2);">
                                ...
                                {{-- Contador de seguidos y seguidores --}}
                                @auth
                                    @if(auth()->id() === $user->id)
                                        @php
                                        ...
                                        <div style="display:flex; gap:1.5rem; margin-top:1rem;">
                                            ...
                                            <div style="position:relative;">
                                                <button onclick="toggleDropdown('following-dropdown')"
                                                    style="background:none; border:none; cursor:pointer; padding:0; text-align:left;">
                                                    <span style="font-size:1rem; font-weight:700; color:var(--beige);">{{ $following->count() }}</span>
                                                    <span style="font-size:.85rem; color:var(--muted); margin-left:.3rem;">siguiendo</span>
                                                </button>
                                                <div id="following-dropdown"
                                                    style="display:none; position:absolute; top:calc(100% + 8px); left:0; width:280px; background:var(--mid); border:1px solid rgba(255,193,147,.2); border-radius:14px; box-shadow:0 16px 48px rgba(0,0,0,.4); z-index:100; max-height:320px; overflow-y:auto;">
                                                    <div
                                                        style="padding:.75rem 1rem; border-bottom:1px solid rgba(255,255,255,.06); font-size:.8rem; font-weight:700; color:var(--beige);">
                                                        Cuentas que sigues</div>
                                                    @if($following->isEmpty())
                                                        <div style="padding:1.5rem; text-align:center; color:var(--muted); font-size:.85rem;">Aún no sigues a nadie.
                                                        </div>
                                                    @else
                                                        @if($followingMusicians->isNotEmpty())
                                                            <div
                                                                style="padding:.5rem 1rem; font-size:.7rem; font-weight:700; text-transform:uppercase; letter-spacing:.07em; color:rgba(255,237,206,.3);">
                                                                Músicos</div>
                                                            @foreach($followingMusicians as $musician)
                                                                <a href="{{ route('profile.show', $musician->username) }}"
                                                                    style="display:flex; align-items:center; gap:.75rem; padding:.75rem 1rem; text-decoration:none; border-bottom:1px solid rgba(255,255,255,.04); transition:background .15s;"
                                                                    onmouseover="this.style.background='rgba(255,193,147,.06)'"
                                                                    ...
                                                                    @else
                                                                        <div
                                                                            style="width:32px; height:32px; border-radius:8px; background:linear-gradient(135deg,var(--orange),var(--red)); display:flex; align-items:center; justify-content:center; font-weight:700; color:#fff; font-size:.85rem; flex-shrink:0;">
                                                                            {{ strtoupper(substr($musician->name, 0, 1)) }}</div>
                                                                    @endif
                                                                    <div>
                                                                        <p style="color:var(--beige); font-size:.85rem; font-weight:600;">{{ $musician->username }}</p>
                                                                        @if($musician->city)
                                                                        <p style="color:var(--muted); font-size:.75rem;">{{ $musician->city }}</p>@endif
                                                                    </div>
                                                                </a>
                                                            @endforeach
                                                        @endif
                                                            ...
                                                            @foreach($followingBands as $band)
                                                                <a href="{{ route('profile.show', $band->username) }}"
                                                                    ...
                                                                    @else
                                                                        <div
                                                                            style="width:32px; height:32px; border-radius:8px; background:linear-gradient(135deg,var(--orange),var(--red)); display:flex; align-items:center; justify-content:center; font-weight:700; color:#fff; font-size:.85rem; flex-shrink:0;">
                                                                            {{ strtoupper(substr($band->name, 0, 1)) }}</div>
                                                                    @endif
                                                                    <div>
                                                                        <p style="color:var(--beige); font-size:.85rem; font-weight:600;">{{ $band->username }}</p>
                                                                        @if($band->city)
                                                                        <p style="color:var(--muted); font-size:.75rem;">{{ $band->city }}</p>@endif
                                                                    </div>
                                                                </a>
                                                            @endforeach
                                                        @endif
                                            ...
                                            {{-- Seguidores --}}
                                            <div style="position:relative;">
                                                <button onclick="toggleDropdown('followers-dropdown')"
                                                    style="background:none; border:none; cursor:pointer; padding:0; text-align:left;">
                                                    <span style="font-size:1rem; font-weight:700; color:var(--beige);">{{ $followers->count() }}</span>
                                                    <span style="font-size:.85rem; color:var(--muted); margin-left:.3rem;">seguidores</span>
                                                </button>
                                                <div id="followers-dropdown"
                                                    style="display:none; position:absolute; top:calc(100% + 8px); left:0; width:280px; background:var(--mid); border:1px solid rgba(255,193,147,.2); border-radius:14px; box-shadow:0 16px 48px rgba(0,0,0,.4); z-index:100; max-height:320px; overflow-y:auto;">
                                                    <div
                                                        style="padding:.75rem 1rem; border-bottom:1px solid rgba(255,255,255,.06); font-size:.8rem; font-weight:700; color:var(--beige);">
                                                        Seguidores</div>
                                                    @if($followers->isEmpty())
                                                        <div style="padding:1.5rem; text-align:center; color:var(--muted); font-size:.85rem;">Aún no tienes
                                                            seguidores.</div>
                                                    @else
                                                            ...
                                                            @foreach($followerMusicians as $follower)
                                                                <a href="{{ route('profile.show', $follower->username) }}"
                                                                    ...
                                                                    @else
                                                                        <div
                                                                            style="width:32px; height:32px; border-radius:8px; background:linear-gradient(135deg,var(--orange),var(--red)); display:flex; align-items:center; justify-content:center; font-weight:700; color:#fff; font-size:.85rem; flex-shrink:0;">
                                                                            {{ strtoupper(substr($follower->name, 0, 1)) }}</div>
                                                                    @endif
                                                                    <div>
                                                                        <p style="color:var(--beige); font-size:.85rem; font-weight:600;">{{ $follower->username }}</p>
                                                                        @if($follower->city)
                                                                        <p style="color:var(--muted); font-size:.75rem;">{{ $follower->city }}</p>@endif
                                                                    </div>
                                                                </a>
                                                            @endforeach
                                                        @endif
                ...
                </div>  {{-- cierre de flex:1 (Info) --}}
            </div>  {{-- cierre de display:flex --}}
        </div>  {{-- cierre de la cabecera --}}
...
{{-- Reproductor de YouTube --}}
        @if($user->account_type === 'musician' && $user->youtube_url)
            @php
    preg_match('/(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]{11})/', $user->youtube_url, $matches);
    $videoId = $matches[1] ?? null;
            @endphp
            @if($videoId)
                <div class="bg-dark rounded-2xl border border-white/10 mt-6" style="overflow:hidden;">
                    <div style="padding:1.25rem 1.5rem; border-bottom:1px solid rgba(255,255,255,.06);">
                        <h2 style="font-family:'Playfair Display',serif; font-size:1.1rem; font-weight:700; color:var(--beige);">Video</h2>
                    </div>
                    <iframe
